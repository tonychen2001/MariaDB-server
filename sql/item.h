...
class Rewritable_query_parameter
{
  ...
  */
  ...
  uint is_empty; // Cut only, do not replace

  bool limit_clause_param;

  Rewritable_query_parameter(uint pos_in_q= 0, uint len_in_q= 0, uint empty= 0)
    : pos_in_query(pos_in_q), len_in_query(len_in_q), is_empty(empty),
...
public:
  ...
  {
    if (copy_up_to(p->pos_in_query) ||
        (p->is_empty ? false : p->append_for_log(thd, dst)))
      return true;
