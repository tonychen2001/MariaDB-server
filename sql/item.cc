                                        ...
                                        const Query_fragment &idx_pos)
{
  ...
  {
    /*
      SELECT var[idx];  -- without a gap between the name and the index
      SELECT var [idx]; -- with a gap between the name and the index
      TODO:
      SELECT (var)[idx] -- ???
    */
    DBUG_ASSERT(pos.end() <= idx_pos.pos());
    DBUG_ASSERT(idx_pos.length());
    uint gap= idx_pos.pos() - pos.end();
    pos= Query_fragment(pos.pos(), pos.length() + gap + idx_pos.length());
    /*
      Mark that neither "this" nor "index" should not be rewritten any more.
  ...
  Item_splocal *res=
    spvar->create_item_splocal_array_element(thd, m_rcontext_handler, index,
                                             pos.pos(), pos.length());
  if (!res)
    return nullptr;
...
void Item_splocal_array_element::print(String *str, enum_query_type query_type)
{
  ...
  Item_splocal::print(str, query_type);
  str->append('[');
  str->append_longlong(m_element_index);
  str->append(']');
}
